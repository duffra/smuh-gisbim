...
<!-- ABOUT THE PROJECT -->
## About The Project
This project is one of the Work Packages for [SMUH](https://smuh.org/info/): Safeguard of Modern Urban Heritage: a cross-disciplinary WebGIS for Knowledge, Monitoring and Risk Analysis.
...
:arrow_forward: You can have a preview of the app, running on Firebase, here: [smuh-gisbim](https://smuh-gisbim.web.app/)
<!-- GETTING STARTED -->
## Getting Started
If you want to set up your project locally, feel free to get a local copy up and run following these simple example steps.

### Prerequisites
You need a basic case study, requiring BIM and GIS resources. Basic samples are included in this repo for testing purposes; if you want to customize the viewer, having a basic understanding of BIM modelling and IFC it's a plus, because you will need to model and export properties according to your specific needs.

These are the kind of files you need to run this application:

* .ifc of your building. _You can use mine for testing purposes_
* .json file defining the properties of your model, extracted by an ifc model using That Open Engine. _You can use mine for testing purposes_
...
1. Get a free API Key for the [Mapbox](https://account.mapbox.com/auth/signup/?page=/) library 
...
1. Make sure you have .ifc files and .json files in the public folder. You can take mines or replace them with your resources.
...
3. Complete and export the array with your buildings in [buildingsList](src/data/SourceList.ts#L33) to setup the resources to be used. This is an example of how to compile the list:
   ...
   ```
4. Go to [Mapbox index file](src/bim-components/Mapbox/index.ts#L81): here is the reference to the mapbox map which will be loaded into the scene and its setup. You can keep mine for testing, or replace it with yours (see [How to setup GIS data on the cloud](#setup-gis-data-on-the-cloud) for further details about map customization)
  ```js
   var myInitialState = {
            pitch: 70.16,
            bearing: 12.77,
...
<!-- HOW TO -->
## How to
This section is dedicated to anyone who wants to customize the app with their own models and data. The following are basic rules that can help you to build your model, export a valid IFC, and run it on a Mapbox map.
### BIM: HowTo 
...
#### Export IFC
It's important to understand what is exported, how and 'where'. Where the properties can be found, and most of all, where the geometry is positioned when exporting IFC. As many software have issues dealing with models far away from the origin, we recommend you to follow this rule: _export the model referred to the local origin and keep georeferencing data (if you have them) in the appropriate properties, without altering the physical position of the model.
...
Furthermore, you need to use a consistent setup to be sure that your data is transferred to IFC without any loss. We recommend you to map properties and build a text file to be used during the export. Below is a schema to highlight the corresponding data between native parameters and IFC props.
...
> 
> You can look at the attached export setup [IFC 4x3](public/support/ExportConfig_FD_IFC4x3.json), valid both for the Geographic Reference tab and the Property Sets one
...
You can have a look at their documentation and build your own tool to extract IFC data to a JSON file, you can use one of the existing open tools developed by the Openers Community, or you can have a look at some functions inserted in this code and experiment with modifying the code by yourself, adding these lines to [convertSelectedIfcToFragments_BackEnd](/src/classes/GeneralTools_Generate.ts#L49)
```js
   /**
...
Our map is centered on Borgo Trento, Verona (Italy), and it is defined to be used with models built on top of our starting file.
```geojson
{
...
```
In this application map is intended as a middle element of the chain: it must be readable by itself as GIS content and must be integrated by BIM models on request.

To achieve this GIS-BIM exchange, every geometry on the map must be enriched by a common attribute in order to get the call for its further data and to integrate the BIM project in the exact position in the GIS environment. The logic is: you ask for a specific building, the app queries the database for geometry and data (IFC side), hides the correspondent GIS geometry, and inserts the BIM model in its geographical location.

To get this working, it is important to keep everything organized and to think the entire GIS-BIM project as a database, where data and geometry fly from the GIS to the BIM side and reverse, whit the possibility to be layered based on specific Levels of Development (LOD).

> [!TIP]
> We opted for an ID identification approach and assigned a unique value to each building. Building ID is the key to exchange data between the GIS and BIM environment.
